...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (testCheck(currentPlayer)) {
			...
			throw new ChessException("(i) You can't put yourself in check!"); 
		}
	...
	public ChessPiece replacePromotedPiece(String type) {
		if (promoted == null) {
			throw new IllegalStateException("(e) Theres no piece to be promoted");
		}
		if (!type.equals("B") && !type.equals("N") && !type.equals("R") && !type.equals("Q")) {
			throw new InvalidParameterException("(i) Invalid Type for Promotion");
		}
	...
	private void validateSourcePosition(Position position) {
		if (!board.thereIsAPiece(position))  {
			throw new ChessException("(i) There is no piece on source position");
		}
		if (currentPlayer != ((ChessPiece)board.piece(position)).getColor()) {
			throw new ChessException("(i) The chosen piece is not yours.");
		}
		if(!board.piece(position).isThereAnyPossibleMove()) {
			throw new ChessException("(i) There is no possible moves for the chosen piece.");
		}
	}
	
	private void validateTargetPosition(Position source, Position target) {
		if (!board.piece(source).PossibleMove(target)) {
			throw new ChessException("(i) The chosen piece can't move to targe position.");
		}
	}
	...
	private ChessPiece king(Color color) {
		...
		throw new IllegalStateException("(e) There is no " + color + " King in the board!");
	}
